...
const addCurrncyList = document.querySelector(".addCurrencyUl");
const currencyList = document.querySelector(".currencyUl");
let currencies = [
    {
        name: "US Dollar",
        abbreviation: "USD",
...
const initialCurrencies = ["GBP", "USD", "EUR", "KRW"];
let filteredCurrencies = initialCurrencies.map(item =>
    currencies.filter(currencies => currencies.abbreviation === item)
);
...
const populateCurrencies = () => {
    currencies.map((currency, i) => {
        addCurrncyList.insertAdjacentHTML(
            "beforeend",
            `
        <li class="addCurrencyLi">
        <div class="flag">
            <img src="${currency.flagURL}" />
        </div>
        <div class="currencyData">
            <p class="currencyName">${currency.abbreviation}</p>
            <p class="baseCurrency">${currency.name}</p>
        </div>
    </li>`
        );
    });
...
const populateCurrencies2 = () => {
    filteredCurrencies.map(currency => {
        ...
        <li class="currencyLi">
        <div class="flag">
            <img src="${currency[0].flagURL}" />
        </div>
        <div class="currencyData">
            <p class="input">
                <span class="currencySymbol">${currency[0].symbol}</span
                ><input type="number" placeholder="0.0000" />
            </p>
            <p class="currencyName">${currency[0].abbreviation} - ${currency[0].name}</p>
            <p class="baseCurrency">1 GBP = 1 GBP</p>
        </div>
    </li>`
        );
    });
